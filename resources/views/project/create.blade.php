<x-page-layout title="Kirim Karya" header="Kirim Karya">
    <section class="max-w-5xl max-md:w-full max-md:p-4 mx-auto h-fit grid grid-cols-2 py-8 gap-4 max-md:grid-cols-1">
        <div class="w-full flex items-center justify-center overflow-hidden border-2 border-gray-200 rounded-md bg-gray-50 max-md:min-h-[100px]">
            <img id="previewImg" class="hidden max-h-[500px]">
            <div id="previewPlaceholder" class="text-xl text-gray-600"><p>Preview Foto</p></div>
        </div>
        ...
        <form class="space-y-4" method="POST" action="{{ route('project.store') }}" enctype="multipart/form-data">
            ...
            <div class="charcounter space-y-1">
                <input type="text" name="title" maxlength="50" placeholder='Judul' class="charcounter-input w-full h-fit pl-2 py-2 border-2 border-gray-200 rounded-md placeholder:text-gray-400 focus:outline-none focus:ring-2 focus:ring-gray-600 bg-gray-50" value="{{ old('title') }}">
            ...
            <div class="charcounter space-y-1">
                <textarea name="description" maxlength="255" placeholder='Deskripsi' class="charcounter-input w-full h-[10rem] pl-2 py-2 border-2 border-gray-200 rounded-md placeholder:text-gray-400 focus:outline-none focus:ring-2 focus:ring-gray-600 bg-gray-50 resize-none">{{ old('description') }}</textarea>
