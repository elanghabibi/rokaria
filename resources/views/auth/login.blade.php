<x-guest-layout title="Masuk">
	<div class="flex items-center justify-center w-full h-screen">
		<form method="POST" action="{{ route('login') }}" class="w-fit h-fit p-8 bg-gray-50 shadow-lg space-y-8 rounded-lg">
			@csrf

			<h1 class="text-2xl text-center font-semibold">Masuk</h1>

			<div class="space-y-4">
				<div>
					<div class="relative">
						<input type="email" name="email" placeholder="Email" class="py-2 focus:outline-none focus:ring-2 focus:ring-gray-400 rounded-md px-10 bg-gray-100 invalid:focus:ring-red-500" value="{{ old('email') }}">
				...
				<button type="submit" class="w-full h-fit py-2 text-gray-50 bg-gray-950 rounded-md text-center cursor-pointer">Masuk</button>
			</div>

			<p class="text-sm text-gray-600 text-center">Belum punya akun? <a href="{{route('register') }}">Daftar!</a></p>
		</form>	
	</div>
</x-guest-layout>